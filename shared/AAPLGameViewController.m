...
static NSString *const SDKSampleAppLicenseKey = @"GOSK-8A45-010C-0EE3-78BD-EEB7";
...
@property (nonatomic, strong) WOWZBroadcast           *broadcast;
@property (nonatomic, strong) WOWZH264Encoder         *encoder;
@property (nonatomic, strong) WOWZAudioDevice         *audioRecorder;
@property (nonatomic, strong) WOWZAACEncoder          *audioEncoder;
...
- (WowzaConfig *) config {
    if (!_config) {
        ...
        if (savedConfig) {
            ...
            _config = [[WowzaConfig alloc] initWithPreset:WOWZFrameSizePreset1280x720];
        }

        _config.broadcastVideoOrientation = WOWZBroadcastOrientationAlwaysLandscapeLeft;
    }

    return _config;
}

- (WOWZBroadcast *) broadcast {
    if (!_broadcast) {
        _broadcast = [WOWZBroadcast new];
...
- (WOWZH264Encoder *) encoder {
    if (!_encoder) {
        _encoder = [WOWZH264Encoder new];
    }

    return _encoder;
}

- (WOWZAudioDevice *) audioRecorder {
    if (!_audioRecorder) {
        _audioRecorder = [WOWZAudioDevice new];
    }

    return _audioRecorder;
}

- (WOWZAACEncoder *) audioEncoder {
    if (!_audioEncoder) {
        _audioEncoder = [WOWZAACEncoder new];
    }
