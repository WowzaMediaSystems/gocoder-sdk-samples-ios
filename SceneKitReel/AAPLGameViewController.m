...
- (WowzaConfig *) config {
    if (!_config) {
        ...
        if (savedConfig) {
            _config = [NSKeyedUnarchiver unarchiveObjectWithData:savedConfig];
        }
        else {
            _config = [[WowzaConfig alloc] initWithPreset:WZFrameSizePreset1280x720];
        }
        
        _config.broadcastVideoOrientation = WZBroadcastOrientationAlwaysLandscape;
    }
...
- (WZBroadcast *) broadcast {
    if (!_broadcast) {
        _broadcast = [WZBroadcast new];
        _broadcast.videoEncoder = self.encoder;
        _broadcast.audioEncoder = self.audioEncoder;
        _broadcast.audioDevice = self.audioRecorder;
    }
    
    return _broadcast;
}

- (WZH264Encoder *) encoder {
    if (!_encoder) {
        _encoder = [WZH264Encoder new];
    }
    
    return _encoder;
}

- (WZAudioDevice *) audioRecorder {
    if (!_audioRecorder) {
        _audioRecorder = [WZAudioDevice new];
    }
    
    return _audioRecorder;
}

- (WZAACEncoder *) audioEncoder {
    if (!_audioEncoder) {
        _audioEncoder = [WZAACEncoder new];
    }
...
- (dispatch_queue_t) goCoderQueue {
    if (!_goCoderQueue) {
        ...
        _goCoderQueue = dispatch_queue_create(goCoderQueueName.UTF8String, DISPATCH_QUEUE_SERIAL);
    }
    return _goCoderQueue;
}
...
- (void) processFrameForGoCoder {
    
    if (!self.licensed) {
        return;
    }
    
    if (self.broadcast.status.state != WZStateRunning) {
        return;
    }
    ...
    if (self.broadcast.status.state == WZStateIdle) {
        self.config.videoWidth = sceneSnapshot.size.width;
        self.config.videoHeight = sceneSnapshot.size.height;
    }
    else {
        
        // NOTE: This is a "simplistic" method for getting the frame buffer bitmap. A real-world implementation would
        // probably get the bitmap directly from the OpenGL context.
        CVPixelBufferRef pixelBuffer = [self pixelBufferFromCGImage:sceneSnapshot.CGImage];
        
        [self.encoder videoFrameWasCaptured:pixelBuffer framePresentationTime:kCMTimeInvalid frameDuration:kCMTimeInvalid];
...
- (void) updateUIControls {
    if (self.broadcast.status.state != WZStateIdle && self.broadcast.status.state != WZStateRunning) {
        ...
        self.settingsButton.enabled     = self.broadcast.status.state == WZStateIdle;
        self.micButton.enabled          = self.broadcast.status.state == WZStateRunning && self.config.audioEnabled;
        self.micButton.hidden           = !self.micButton.enabled;
    }
}

#pragma mark - WowzaGoCoder Actions -

- (IBAction) didTapGoCoderSettings:(id)sender {
...
- (IBAction) didTapBroadcastButton:(id)sender {
    ...
    NSError *configError = [self.config validateForBroadcast];
    if (configError != nil) {
        [self showGoCoderAlertWithTitle:@"Incomplete Streaming Settings" error:configError];
        return;
    }
    ...
    if (self.broadcast.status.state == WZStateRunning) {
        ...
        dispatch_async(self.goCoderQueue, ^{
            [self.broadcast startBroadcast:self.config statusCallback:self];
        });
    }
...
- (IBAction) didTapSettingsButton:(id)sender {
    ...
    [settingsVC addDisplaySection:SettingsViewSectionBandwidthThrottling];
    
    SettingsViewModel *settingsModel = [[SettingsViewModel alloc] initWithSessionConfig:self.config];
...
- (void) onWZStatus:(WZStatus *) goCoderStatus {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.broadcast.status.state == WZStateIdle || self.broadcast.status.state == WZStateRunning) {
            [self.broadcastButton setImage:[UIImage imageNamed:(self.broadcast.status.state == WZStateIdle) ? @"start_button" : @"stop_button"] forState:UIControlStateNormal];
        }
...
- (void) onWZError:(WZStatus *) goCoderStatus {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [self showGoCoderAlertWithTitle:@"Live Streaming Error" error:goCoderStatus.error];
        
        [self updateUIControls];
    });
}

#pragma mark - WowzaGoCoder Segues -

- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.destinationViewController isKindOfClass:[UINavigationController class]]) {
        if ([((UINavigationController *)segue.destinationViewController).topViewController isKindOfClass:[SettingsViewController class]]) {
            SettingsViewController *settingsVC = (SettingsViewController *)(((UINavigationController *)segue.destinationViewController).topViewController);
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    NSData *savedConfigData = [NSKeyedArchiver archivedDataWithRootObject:self.config];
...
- (void)setup
{
    ...
    NSError *goCoderLicensingResult = [WowzaGoCoder registerLicenseKey:SDKSampleAppLicenseKey];
    self.licensed = goCoderLicensingResult == nil;
...
#pragma mark - WowzaGoCoder Start a 3fps timer
